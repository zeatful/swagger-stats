...
swsAPIStats.prototype.normalizePath = function (originalUrl, normalizationMasks = [], placeholder = '#val') {
    const path = new URL(originalUrl);
    const urlParser = new UrlValueParser({ extraMasks: normalizationMasks });
    return path ? urlParser.parsePathValues(path, placeholder) : 'FAILED TO PARSE';
};
...
swsAPIStats.prototype.matchRequest = function (req) {
    ...
    // if swaggerOnly = false, no swaggerSpec provided, and pathNormalization enabled
    if(swsSettings.pathNormalization && !swsSettings.swaggerOnly && !swsSettings.swaggerSpec){
        url = this.normalizePath(url, swsSettings.normalizationMasks);
    }
